...
def setup():
    ...
    for _ in range(pontos_iniciais):
        ...
        pontos += Ponto(x, y, cor)  # acrescenta um Ponto
...
def mouse_clicked():  # Sob clique do mouse seleciona/deseleciona Pontos ou Arestas
...
def key_pressed():   # Quando uma tecla é pressionada
    ...
    if key == ' ':
        # acrescenta Ponto no set
        pontos += Ponto(mouse_x, mouse_y, cor_rnd())
    # 'd' remove os Pontos previamente selecionandos com clique, marcados em preto.
    if key == 'd' and len(pontos) > 1:
        for ponto in pontos:
            if ponto.sel and len(
                    pontos) > 1:  # se a lista tiver pelo menos 2 pontos
...
def mouse_dragged():        # quando o mouse é arrastado
    for ponto in pontos:   # para cada Ponto checa distância do mouse
        if dist(mouse_x, mouse_y, ponto.x, ponto.y) < tamanho * 3 / 2:
            ponto.x, ponto.y = mouse_x, mouse_y    # move o Ponto para posição do mouse
...
def point_inside_line(
    ...
    dir -= theLineEndPoint1
    diff = PVector(thePoint.x, thePoint.y)
    diff -= theLineEndPoint1
