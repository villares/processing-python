...
def setup():
    global fundo
    fundo = load_image("fundo22.png")
    size(600, 600, P3D)
    cam = PeasyCam(this, 190)
    cam.set_minimum_distance(190)
    cam.set_maximum_distance(190)
...
def draw():
    ...
    if not frame_count % 3 and degrees(ANG) < 360:
        # saveFrame("f###.png") # descomente esta linha para salvar frames!
        print((frame_count, degrees(ANG)))
