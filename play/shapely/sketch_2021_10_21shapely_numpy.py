...
#         77        10   93564129.0 9356412.9    100.0      list(map
#        77        10  134511180.0 13451118.0    100.0      list(map(grow, unvisited))
#        78        10  115443641.0 11544364.1    100.0      list(map(grow, unvisited))

def key_pressed():
    print(len(unvisited_tris), len(tris))
    unvisited = unvisited_tris[:]
    unvisited_tris[:] = [] 
    tuple(map(grow, unvisited))
    tris.extend(unvisited_tris)
...
def min_max(points):
    ...
    return (min(xa, xb, xc), min(ya, xb, yc)), (max(xa, xb, xc), max(ya, xb, yc))
